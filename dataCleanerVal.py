...
for sample in os.listdir(data_dir):
    ...
    first_start = min(starts) # earliest label
    last_end = max(ends) + 1
    # slice off both prefix and suffix
    accel_df = accel_df.iloc[first_start:last_end].reset_index(drop=True)
    gyro_df  = gyro_df.iloc[first_start:last_end].reset_index(drop=True)
    ...
    labels_df['t_x'] = [c - first_start for c in centers]
